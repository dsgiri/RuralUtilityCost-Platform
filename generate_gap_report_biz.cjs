...
const bizCalculators = [
    'GrantReadiness', 'CutCost', 'HabitatCost', 'PainPointPriority', 'EnergyDemand', 'ExpandProfit', 'GrantFinder', 'FoodProcessingCompliance'
];
const results = [];
files.forEach(file => {
  ...
  if (fileName !== `${dirName}.tsx` || !bizCalculators.includes(dirName)) return;
...
const output = [];
output.push("Business Calculators Gap Analysis\n");
results.forEach(g => {
  const gaps = [];
...
fs.writeFileSync('GAP_BIZ.md', output.join('\n'));
